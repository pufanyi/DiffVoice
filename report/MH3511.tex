...
\title{DiffVoice: Spot the Differences in Voice}
...
\begin{table}
	...
	\begin{tabular}{ll}
		...
		\hline
		\texttt{meanfreq} & Average frequency (kHz) \\
		\texttt{sd} & Frequency standard deviation \\
		\texttt{median} & Median frequency (kHz) \\
		\texttt{Q25} & First quartile (kHz) \\
		\texttt{Q75} & Third quartile (kHz) \\
		\texttt{IQR} & Interquartile range (kHz) \\
		\texttt{skew} & Skewness of the frequency distribution \\
		\texttt{kurt} & Kurtosis of the frequency distribution \\
		\texttt{sp.ent} & Spectral entropy \\
		\texttt{sfm} & Spectral flatness measure \\
		\texttt{mode} & Mode frequency \\
		\texttt{centroid} & Frequency centroid \\
		\texttt{meanfun} & Mean fundamental frequency across the signal \\
		\texttt{minfun} & Minimum fundamental frequency across the signal \\
		\texttt{maxfun} & Maximum fundamental frequency across the signal \\
		\texttt{meandom} & Mean dominant frequency across the signal \\
		\texttt{mindom} & Minimum dominant frequency across the signal \\
		\texttt{maxdom} & Maximum dominant frequency across the signal \\
		\texttt{dfrange} & Dominant frequency range \\
		\texttt{modindx} & Modulation index \\
		\texttt{gender} & Modulation index \\
		\hline
	\end{tabular}
