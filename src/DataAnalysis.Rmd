...
We found that there is data that equals to 0 in the `median` column. Therefore, we need to remove the outliers first

```{r}
IQR = IQR(median)
outlr = c(abs(median - mean(median)) > 1.5 * IQR)
#append the outliers and id to the data
outlr_id = voice$id[outlr]
median_without_outliers = median[!outlr]
```

Then, we save the IDs of the outliers to a CSV file.

```{r}
write.csv(outlr_id, "../data/outliers/median.csv", row.names = FALSE)
```

```{r}
hist(median_without_outliers, breaks = 80, main = "Histogram of median without Outliers", xlab = "median", probability = TRUE)
```
